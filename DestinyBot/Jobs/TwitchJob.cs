    ...
    public class TwitchJob : IJob
    {
        ...
        public void Execute()
        {
            using (_db)
            {
                ...
                if (DateTimeOffset.UtcNow - DateTimeOffset.FromUnixTimeSeconds(streamer.StreamEndTime) <
                    TimeSpan.FromMinutes(3)) return;
                ...
                if (isStreaming && streamer.StreamLength <= TimeSpan.Zero)
                {
                    ...
                    foreach (var subscription in streamer.TwitchSubscriptions)
                    {
                        ...
                        message.ModifyAsync(x => x.Embed = CreateTwitchEmbed(streamer, subscription, stream, logo))
                            .GetAwaiter()
                            .GetResult();
                    }
                }
            ...
            string logoUrl)
        {
            ...
            return new EmbedBuilder()
                .WithAuthor($"{streamer.Name} is live",
                    url: subscription.AlternateLink ?? $"https://twitch.tv/{streamer.Name}")
